...
class ImageViewer:
    ...
    def __init__(self, qlabel):
        ...
        self.orig_img = qlabel                         # widget/window name where image is displayed (I'm usiing qlabel)
        ...
        self.orig_img.setSizePolicy(QtWidgets.QSizePolicy.Ignored, QtWidgets.QSizePolicy.Ignored)
    ...
    def loadImage2(self, imagePath):
        ''' To load and display new image.'''
        self.qimage = QImage(imagePath)
        self.qpixmap = QPixmap(self.orig_img.size())
        if not self.qimage.isNull():
            # reset Zoom factor and Pan position
            self.zoomX = 1
            self.position = [0, 0]
            self.qimage_scaled = self.qimage.scaled(self.orig_img.width(), self.orig_img.height(), QtCore.Qt.KeepAspectRatio)
            self.update2()
        else:
            QtWidgets.QMessageBox.warning(self, 'Invalid Crop', 'Crops in the image are all Invalid')
    ...
    def update2(self):
        ...
        if not self.qimage_scaled.isNull():
            # check if position is within limits to prevent unbounded panning.
            px, py = self.position
            px = px if (px <= self.qimage_scaled.width() - self.orig_img.width()) else (self.qimage_scaled.width() - self.orig_img.width())
            py = py if (py <= self.qimage_scaled.height() - self.orig_img.height()) else (self.qimage_scaled.height() - self.orig_img.height())
            px = px if (px >= 0) else 0
            py = py if (py >= 0) else 0
            self.position = (px, py)
            ...
            painter.begin(self.qpixmap)
            painter.drawImage(QtCore.QPoint(0, 0), self.qimage_scaled,
                    QtCore.QRect(self.position[0], self.position[1], self.orig_img.width(), self.orig_img.height()) )
            painter.end()

            self.orig_img.setPixmap(self.qpixmap)
        else:
            pass
